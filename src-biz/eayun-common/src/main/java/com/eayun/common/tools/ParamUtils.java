...
public class ParamUtils {
	 ...
	 * @return 返回提交传递的值
	 */
	public static String getParameter1(HttpServletRequest request, String param) {
		String str = request.getParameter(param);
		if (str != null) {
			return str.trim();
		} else {
	 ...
	 * @return
	 */
	private static String filterSpecial(String values) {
		if (values == null)
			return null;
		values = values.replaceAll("\\%28", "（");
		values = values.replaceAll("\\)", "）");
		values = values.replaceAll("\\;", "；");
		values = values.replaceAll("\\&", "&amp;");
		values = values.replaceAll("\\>", "&gt;");
		values = values.replaceAll("\\\"", "&quot;");
		values = values.replaceAll("\\'", "&frasl");
		values = values.replaceAll("\\%27", "&frasl");
		return values;
	}
}
