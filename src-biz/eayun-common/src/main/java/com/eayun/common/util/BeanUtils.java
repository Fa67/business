...
public class BeanUtils extends org.apache.commons.beanutils.BeanUtils {
    private static final Logger log = LoggerFactory.getLogger(com.eayun.common.util.BeanUtils.class);
	private BeanUtils() {
	 ...
	 */
	public static void copyMap(Map dest, Map orig, String propPref) {
		try {
			for (Iterator names = orig.keySet().iterator(); names.hasNext();) {
				String name = (String) names.next();
				if(name.startsWith(propPref+".")){
					Object value = orig.get(name);
					dest.put(name.replace(propPref+".", ""), value);
				}
			}
		} catch (Exception e) {
		    log.error(e.getMessage(),e);
