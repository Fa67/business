...
public class CdnMonthSummaryThread implements Runnable {
	
	private final Logger log = LoggerFactory.getLogger(CdnMonthSummaryThread.class);

	private MongoTemplate mongoTemplate;
	
	private Date now;
	
	public CdnMonthSummaryThread(MongoTemplate mongoTemplate, Date now) {
	...
	@Override
	public void run() {
		log.info("汇总客户上个月的CDN下载流量、动态请求数和HTTPS请求数");
		try {
			...
			Date lastMonth = DateUtil.addDay(endTime, new int[]{0,-1});
			
			Aggregation agg = Aggregation.newAggregation(
			        Aggregation.match(Criteria.where("timestamp").gt(lastMonth)),
			...
			if(totallist.size() > 0){
				for(int i = 0; i < totallist.size(); i++){
					...
					insertObj.put("timestamp", lastMonth);
					...
					mongoTemplate.insert(insertObj, MongoCollectionName.OBS_CDN_1MON);
				}
			}
		} catch (Exception e) {
